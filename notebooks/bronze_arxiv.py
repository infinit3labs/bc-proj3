...
path = f"/mnt/bronze/arxiv"
path

# COMMAND ----------

# Arxiv file format uses '-' as the date separator, other files use '_'
def get_run_date_files(run_date, path):
    assert len(run_date) == 8
    assert len(path) > 0
    _run_date_fmt = run_date[:4] + '-' + run_date[4:6] + '-' + run_date[-2:]
    _files = []
...
out_df = spark.sql(f"""
with data as (
       select explode(feed.entry) results,
              source_file_name
       from df
     )
select results.*,
...
# MAGIC %sql
# MAGIC create or replace table main.bronze.arxiv
# MAGIC select *
# MAGIC from out_df
